...
(function () {
    ...
    let mainHeader = null;
    let mainFooter = null;
    let isNavClickActive = false; // Flag to prevent scroll from overriding click highlight
    ...
    function initSmoothScroll() {
        document.querySelectorAll('nav a[href^="#"]').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                ...
                // Set flag to prevent scroll from overriding click highlight
                // This flag will be reset when user clicks another section
                ...
                if (!href || href === '#' || href === '#home') {
                    ...
                    // Set flag temporarily, then allow scroll-based updates
                    isNavClickActive = true;
                    setTimeout(() => {
                        isNavClickActive = false;
                        updateActiveNav(); // Update after scroll completes
                    }, 1000);
                    return;
                }
                ...
                if (target) {
                    // Get nav height for accurate offset calculation
                    ...
                    // Calculate exact position - section top minus nav height
                    const offsetTop = Math.max(0, sectionTop - navHeight);
                    
                    // Scroll to exact position with smooth behavior
                    window.scrollTo({ 
                        top: offsetTop, 
                        behavior: 'smooth' 
                    });
    ...
    function updateActiveNav() {
        // Don't update if user just clicked on a nav link - keep the click highlight temporarily
        if (isNavClickActive) {
            return;
        }
        ...
        // If at top of page (first 300px), always show Home as active
        if (scrollPos < 300) {
            currentSection = 'home';
        } else {
            // Find which section is currently in view
            // Use a more accurate calculation based on viewport
            let bestMatch = null;
            ...
            sections.forEach(section => {
                ...
                // Check if section is in viewport (with nav offset)
                const viewportTop = scrollPos + navHeight;
                ...
                // If section is significantly visible, consider it active
    ...
    function handleScroll() {
        // Don't update nav if user clicked on a section - keep the click highlight
        // The flag will only be reset when user clicks another section
        if (isNavClickActive) {
            return;
        }
        if (scrollTimeout) {
